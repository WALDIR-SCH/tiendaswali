...
interface Producto {
  ...
  procesador: string; especificaciones_tecnicas: string | Record<string, string>;
  unidad_venta: string; precio_caja: number; precio_unitario: number;
  precio_oferta_caja: number | null; precio_oferta_unidad: number | null;
  en_oferta: boolean; en_oferta_unidad: boolean; precios_volumen?: PreciosVolumen;
  stock_cajas: number; stock_unidades: number; unidades_por_caja: number;
  pedido_minimo: number; stock_minimo_cajas: number; moneda: string;
  peso_kg: number; dimensiones: string; garantia_meses: number;
  imagen_principal: string; documento_ficha: string; estado: string;
  imagenes?: string[]; rating_promedio?: number; total_resenas?: number;
  destacado?: boolean; sim?: string; conectividad?: string; bateria_mah?: string;
  camara_principal?: string; camara_frontal?: string; tamano_pantalla?: string; version_so?: string;
}
interface Resena {
  id: string; usuario: string; usuarioEmail: string; usuarioId: string;
  usuarioFoto: string | null; comentario: string; rating: number;
  imagenes: string[]; fecha: any; verificado: boolean; util: number;
}

const C = {
  orange: "#FF6600", yellow: "#F6FA00", green: "#28FB4B", greenDark: "#16a34a",
  purple: "#9851F9", purpleDark: "#7c3aed", black: "#000000", white: "#FFFFFF",
  gray100: "#F3F4F6", gray200: "#E5E7EB", gray300: "#D1D5DB", gray400: "#9CA3AF",
    ...
    <div className="relative overflow-hidden rounded-xl">
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2">
        ...
        <button onClick={auto ? stop : start} className="px-4 py-2 rounded-lg text-white text-sm font-bold flex items-center gap-2"
          ...
          <RotateCw size={14} className={auto ? "animate-spin" : ""} />{auto ? "Detener" : "360°"}
        </button>
...
const QuantitySelector = ({ quantity, setQuantity, max, tipo, pedidoMinimo, disabled = false }: {
  ...
  const maxR   = Math.min(Math.max(max, 0), 99999);
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="flex items-center rounded-xl overflow-hidden border-2 transition-colors"
        ...
        <button onClick={dec} disabled={disabled || atMin}
          className="px-4 py-3 font-black text-lg transition-all select-none"
          style={{ background: C.gray100, color: disabled || atMin ? C.gray300 : C.gray600,
            cursor: disabled || atMin ? "not-allowed" : "pointer",
            borderRight: `1px solid ${C.gray200}`, minWidth: 48 }}
          onMouseEnter={e => { if (!disabled && !atMin) (e.currentTarget as HTMLElement).style.background = C.gray200; }}
          onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = C.gray100; }}>−</button>
        <input type="number" value={quantity} min={minVal} max={maxR} onChange={handleChange} disabled={disabled}
          className="flex-1 text-center bg-white py-3 font-black text-gray-900 focus:outline-none"
          style={{ fontSize: 16, MozAppearance: "textfield" as any, WebkitAppearance: "none" as any }} />
        <button onClick={inc} disabled={disabled || atMax}
          className="px-4 py-3 font-black text-lg transition-all select-none"
          style={{ background: C.gray100, color: disabled || atMax ? C.gray300 : C.gray600,
            cursor: disabled || atMax ? "not-allowed" : "pointer",
            borderLeft: `1px solid ${C.gray200}`, minWidth: 48 }}
      ...
      <div className="flex justify-between text-[11px] px-0.5" style={{ color: C.gray400 }}>
        <span>Mínimo: <strong style={{ color: C.purple }}>{minVal} {tipo === "caja" ? "caja" : "uds"}</strong></span>
        <span>Disponible: <strong style={{ color: maxR > 0 ? C.greenDark : "#dc2626" }}>{maxR} {tipo === "caja" ? "cajas" : "uds"}</strong></span>
      </div>
    </div>
  );
};
...
const StarRating = ({ rating, setRating, readonly = false, size = 20 }: {
  ...
  <div className="flex gap-0.5">
    {[1, 2, 3, 4, 5].map(s => (
      <button key={s} type="button" onClick={() => !readonly && setRating?.(s)} disabled={readonly}
    ...
    <div className="rounded-xl border-2 overflow-hidden" style={{ borderColor: isOut ? "#fecaca" : isCrit ? "#fde68a" : "#bbf7d0" }}>
      ...
      <div className="p-4 bg-white grid grid-cols-2 gap-3">
        <div className="rounded-lg p-3 border" style={{ borderColor: C.gray200, background: C.gray100 }}>
          ...
          <p className="text-[10px] font-bold mt-1" style={{ color: C.orange }}>= {stockCajas * udsXcaja} unidades</p>
        </div>
        <div className="rounded-lg p-3 border" style={{ borderColor: C.gray200, background: C.gray100 }}>
          <div className="flex items-center gap-1.5 mb-2">
          ...
          <p className="text-[10px] font-bold mt-1 text-gray-400">{udsXcaja} uds por caja</p>
        </div>
      </div>
...
const ResumenPrecio = ({ precio, cantidad, tipo, unidadesPorCaja, moneda }: {
  precio: number; cantidad: number; tipo: "caja" | "unidad"; unidadesPorCaja: number; moneda: string;
}) => {
      ...
      <div className="p-4 bg-white space-y-2.5">
        ...
        {tipo === "caja" && (
          <div className="flex justify-between text-xs text-gray-400">
            <span>Precio por unidad</span><span>{fmtPEN(precioPorUd)}</span>
          </div>
        )}
              ...
              return (
                <button key={i} type="button" onClick={() => onSelectPrecio(e.precio, e.cantidad)}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-black ${sel ? "text-gray-900" : "text-gray-700"}`}>{fmtPEN(e.precio)}</span>
                  </div>
                </button>
              );
            })}
          </div>
          <p className="text-[10px] text-gray-400 mt-3">* Precios sin IGV.</p>
        </div>
      )}
    </div>
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          const p: Producto = {
            id: snap.id,
            sku: d.sku||"", nombre_producto: d.nombre_producto||"", descripcion_corta: d.descripcion_corta||"",
            categoria_id: d.categoria_id||"", marca: d.marca||"", modelo: d.modelo||"", color: d.color||"",
            capacidad_almacenamiento: d.capacidad_almacenamiento||"", capacidad_ram: d.capacidad_ram||"",
            sistema_operativo: d.sistema_operativo||"", version_so: d.version_so||"", procesador: d.procesador||"",
            especificaciones_tecnicas: d.especificaciones_tecnicas||"", unidad_venta: d.unidad_venta||"",
            precio_caja: Number(d.precio_caja)||0, precio_unitario: Number(d.precio_unitario)||0,
            precio_oferta_caja: d.precio_oferta_caja||null, precio_oferta_unidad: d.precio_oferta_unidad||null,
            en_oferta: d.en_oferta||false, en_oferta_unidad: d.en_oferta_unidad||false,
            precios_volumen: d.precios_volumen||undefined,
            stock_cajas: Number(d.stock_cajas)||0, stock_unidades: Number(d.stock_unidades)||0,
            unidades_por_caja: Number(d.unidades_por_caja)||10, pedido_minimo: Number(d.pedido_minimo)||5,
            stock_minimo_cajas: Number(d.stock_minimo_cajas)||2, moneda: "PEN",
            peso_kg: Number(d.peso_kg)||0, dimensiones: d.dimensiones||"",
            garantia_meses: Number(d.garantia_meses)||0, imagen_principal: d.imagen_principal||"",
            documento_ficha: d.documento_ficha||"", estado: d.estado||"Inactivo",
            ...
            destacado: d.destacado||false, sim: d.sim||"", conectividad: d.conectividad||"",
            bateria_mah: d.bateria_mah||"", camara_principal: d.camara_principal||"",
            camara_frontal: d.camara_frontal||"", tamano_pantalla: d.tamano_pantalla||"",
          };
          setProducto(p);
    ...
    const unsub = onSnapshot(q, snap => {
      const data: Resena[] = snap.docs.map(d => ({
        id: d.id, usuario: d.data().usuario||"Usuario", usuarioEmail: d.data().usuarioEmail||"",
        usuarioId: d.data().usuarioId||"", usuarioFoto: d.data().usuarioFoto||null,
        comentario: d.data().comentario||"", rating: d.data().rating||5,
        imagenes: Array.isArray(d.data().imagenes) ? d.data().imagenes : [],
    ...
    <div className="min-h-screen bg-white text-gray-800 antialiased">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 lg:py-10">
            ...
            <div className="relative aspect-square rounded-2xl overflow-hidden border-2" style={{ borderColor: C.gray200 }}>
              ...
              {productImages.length > 1 && (
                <button onClick={() => setIsView360(!isView360)}
                  ...
                  <RotateCw size={13} />{isView360 ? "Vista normal" : "360°"}
                </button>
              )}
              ...
              <div className="absolute top-14 right-3 flex flex-col gap-1.5">
                {producto.en_oferta && <span className="text-[10px] font-black px-2 py-1 rounded-full text-white" style={{ background: C.orange }}>OFERTA CAJA</span>}
                {producto.en_oferta_unidad && <span className="text-[10px] font-black px-2 py-1 rounded-full text-black" style={{ background: C.yellow }}>OFERTA UNIDAD</span>}
                {producto.destacado && <span className="text-[10px] font-black px-2 py-1 rounded-full text-white" style={{ background: C.purple }}>DESTACADO</span>}
              </div>
            </div>
            <div className="flex gap-2 overflow-x-auto pb-1">
              {productImages.map((img, i) => (
                <button key={i} onClick={() => setImgSeleccionada(img)}
                  ...
                  style={{ borderColor: imgSeleccionada === img ? C.purple : C.gray200,
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            {(producto.capacidad_almacenamiento||producto.capacidad_ram||producto.sistema_operativo||producto.procesador) && (
              <div className="flex flex-wrap gap-2">
                {producto.capacidad_almacenamiento && <span className="text-xs font-bold px-2.5 py-1 rounded-full text-white" style={{ background: C.purple }}>{producto.capacidad_almacenamiento}</span>}
                {producto.capacidad_ram            && <span className="text-xs font-bold px-2.5 py-1 rounded-full text-black" style={{ background: C.yellow }}>RAM {producto.capacidad_ram}</span>}
                {producto.sistema_operativo        && <span className="text-xs font-bold px-2.5 py-1 rounded-full text-black" style={{ background: C.green }}>{producto.sistema_operativo}</span>}
                {producto.procesador               && <span className="text-xs font-bold px-2.5 py-1 rounded-full text-white" style={{ background: C.orange }}>{producto.procesador}</span>}
              </div>
            )}

            <StockDisplay producto={producto} tipo={tipoCompra} />
            <TipoCompraSelector tipo={tipoCompra} setTipo={tipo => {
              setTipoCompra(tipo); setCantidad(tipo === "caja" ? 1 : producto.pedido_minimo);
            ...
            <div className="rounded-xl p-4 border-2" style={{ background: C.gray100, borderColor: C.gray200 }}>
              <div className="flex items-baseline gap-2 flex-wrap mb-2">
                ...
                {precioSeleccionado !== precioBase && <span className="text-sm text-gray-400 line-through">{fmtPEN(precioBase)}</span>}
                <span className="text-xs font-bold px-2 py-0.5 rounded-full text-gray-500 border" style={{ borderColor: C.gray300 }}>sin IGV</span>
              </div>
              <div className="flex flex-wrap gap-3 text-xs text-gray-600">
                {tipoCompra === "caja" ? (
                  ...
                  <span className="flex items-center gap-1 font-bold" style={{ color: C.purple }}>{fmtPEN(precioSeleccionado/producto.unidades_por_caja)} / unidad</span></>
                ) : (
                  <><span className="flex items-center gap-1"><Box size={12} style={{ color: C.orange }} /> Por unidad suelta</span>
                  <span className="flex items-center gap-1 font-bold" style={{ color: C.orange }}>Mín {producto.pedido_minimo} unidades</span></>
                )}
              </div>
            </div>
            ...
            />
            <QuantitySelector quantity={cantidad} setQuantity={setCantidad} max={stockActual}
            ...
            {stockActual > 0 && producto.estado === "Activo" && (
              <ResumenPrecio precio={precioSeleccionado} cantidad={cantidad}
                tipo={tipoCompra} unidadesPorCaja={producto.unidades_por_caja} moneda={producto.moneda} />
            )}
            ...
            <div className="flex gap-3">
              <button onClick={handleAgregarAlCarrito}
                ...
                style={{ background: agregado ? "#16a34a" : stockActual<=0||producto.estado!=="Activo" ? C.gray400 : `linear-gradient(135deg,${C.purple},${C.purpleDark})`,
              ...
              <button onClick={toggleWishlist} disabled={wishlistLoading || !currentUser}
                ...
                style={{ background: enWishlist ? C.gray100 : C.white,
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={title} className="rounded-xl p-3 border-2 hover:bg-gray-50 transition-colors" style={{ borderColor: C.gray200 }}>
        ...
        <div className="mt-12">
          ...
          {activeTab === "descripcion" && (
            <div className="rounded-2xl p-6 border-2" style={{ background: C.gray100, borderColor: C.gray200 }}>
              ...
              {(producto.sim||producto.conectividad||producto.bateria_mah||producto.camara_principal) && (
                <div className="mt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {[["SIM",producto.sim],["Conectividad",producto.conectividad],["Batería",producto.bateria_mah],
                      ["Cám. Trasera",producto.camara_principal],["Cám. Frontal",producto.camara_frontal],["Pantalla",producto.tamano_pantalla]
                    ].filter(([,v])=>v).map(([k,v])=>(
                      <div key={k as string} className="flex justify-between p-2.5 rounded-xl border-2 bg-white" style={{ borderColor: C.gray200 }}>
              ...
              <div className="mt-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {[["SKU",producto.sku],["Marca",producto.marca],["Modelo",producto.modelo||"N/A"],
                    ["Color",producto.color||"N/A"],["Almacenamiento",producto.capacidad_almacenamiento||"N/A"],
                    ["RAM",producto.capacidad_ram||"N/A"],
                    ["SO",`${producto.sistema_operativo} ${producto.version_so||""}`.trim()||"N/A"],
                    ["Procesador",producto.procesador||"N/A"],["Garantía",`${producto.garantia_meses} meses`],
              ...
              {specsArray.length > 0 ? (
                <div className="space-y-2">
                  {specsArray.map((item, i) => (
                    <div key={i} className="rounded-xl p-3 border-2 bg-white hover:bg-gray-50 transition-colors" style={{ borderColor: C.gray200 }}>
                      <div className="flex items-start gap-2">
          ...
          {/* ══ OPINIONES — formulario de escritura ELIMINADO ══ */}
          {activeTab === "feedback" && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">

              {/* Columna izquierda: resumen + aviso */}
              <div className="space-y-4">
                      ...
                      <div key={s} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 h-2 rounded-full overflow-hidden bg-gray-100">
                          <div className="h-full rounded-full"
                            style={{ width: `${porcentajes[s as keyof typeof porcentajes]}%`,
                ...
                {/* Aviso: dónde dejar reseña */}
                <div className="rounded-2xl p-4 border-2" style={{ background: `${C.purple}06`, borderColor: `${C.purple}20` }}>
                  <div className="flex items-start gap-3">
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
                      style={{ background: `${C.purple}15` }}>
                      <BadgeCheck size={17} style={{ color: C.purple }} />
                    </div>
                    <div>
                      <p className="text-xs font-black text-gray-900 mb-1">Reseñas verificadas</p>
                      <p className="text-[10px] leading-relaxed" style={{ color: C.gray500 }}>
                        Solo clientes que compraron y recibieron este producto pueden opinar.
                        Puedes dejar tu reseña desde{" "}
                        <Link href="/opciones/pedidos" className="font-bold underline" style={{ color: C.purple }}>
                          Mis Pedidos
                        </Link>
                        {" "}una vez que tu pedido esté entregado.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {reseñas.length > 0 ? (
                  ...
                  <div className="text-center py-12 rounded-2xl border-2" style={{ background: C.gray100, borderColor: C.gray200 }}>
                    ...
                    <p className="text-xs text-gray-400">
                      Las reseñas aparecerán aquí cuando los clientes las publiquen desde sus pedidos entregados.
                    </p>
                  </div>
                )}
              </div>

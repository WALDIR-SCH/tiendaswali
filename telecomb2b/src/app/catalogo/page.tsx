...
const ProductoCard = ({
  ...
  return (
    <div className="group relative cursor-pointer rounded-2xl overflow-hidden border-2 transition-all duration-300 bg-white"
      ...
      <div className="p-4">
        {/* Marca + nombre + botón WhatsApp en la esquina (cuadro rojo) */}
        <div className="flex items-start justify-between gap-2 mb-1.5">
          ...
          {/* ── BOTÓN WHATSAPP ── */}
          <a
            href={`https://wa.me/51925903712?text=${encodeURIComponent(
              `Hola, me interesa este producto 👇\n\n*${prod.nombre_producto}*\nMarca: ${prod.marca}\nCategoría: ${prod.categoria_id}${prod.sku ? `\nSKU: ${prod.sku}` : ""}${prod.capacidad_almacenamiento ? `\nAlmac.: ${prod.capacidad_almacenamiento}` : ""}${prod.capacidad_ram ? ` · RAM: ${prod.capacidad_ram}` : ""}${prod.color ? `\nColor: ${prod.color}` : ""}\nPrecio/caja: ${fmtPEN(prod.en_oferta && prod.precio_oferta_caja ? prod.precio_oferta_caja : prod.precio_caja || 0)}\nStock: ${prod.stock_cajas || 0} cajas disponibles\n\n¿Pueden darme más información?`
            )}`}
            target="_blank"
            rel="noopener noreferrer"
            onClick={e => e.stopPropagation()}
            title="Consultar por WhatsApp"
            className="shrink-0 flex items-center justify-center rounded-xl transition-all hover:scale-110 active:scale-95"
            style={{
              width: "44px",
              height: "44px",
              background: "#25D366",

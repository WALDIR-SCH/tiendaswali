...
/* ── Item line component — muestra caja o unidad con desglose ── */
const ItemLinea = ({ item, esEntregado, calificado, onCalificar }: {
  item: any; esEntregado: boolean; calificado: boolean; onCalificar: () => void;
}) => {
  const esCaja          = item.tipoCompra === "caja" || item.unidad_venta?.toLowerCase().includes("caja");
  const udsPorCaja      = Number(item.unidadesPorCaja || item.unidades_por_caja) || 10;
  const cantidad        = Number(item.cantidad) || 1;
  const precioUnit      = Number(item.precioBase || item.precio) || 0;
  const subtotal        = precioUnit * cantidad;
  const totalUnidades   = esCaja ? cantidad * udsPorCaja : cantidad;
  const imagen          = item.imagenUrl || item.imagen_principal;
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl border transition-all"
      ...
      <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 flex items-center justify-center"
        ...
        {imagen
          ? <img src={imagen} alt={item.nombre} className="w-full h-full object-contain p-1" />
          : <Package size={18} style={{ color: C.gray300 }} />}
      </div>

      {/* Info */}
      <div className="flex-1 min-w-0">
        <p className="text-xs font-bold truncate" style={{ color: C.gray900 }}>{item.nombre}</p>
        <p className="text-[10px] mt-0.5" style={{ color: C.gray500 }}>SKU: {item.sku || "—"}</p>

        {/* Tipo compra + desglose */}
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          ...
          {/* Cantidad */}
          <span className="text-[10px] font-bold" style={{ color: C.gray700 }}>
            {esCaja
              ? `${cantidad} caja${cantidad !== 1 ? "s" : ""} × ${udsPorCaja} uds`
          ...
          {esCaja && (
            <span className="text-[10px] font-black px-1.5 py-0.5 rounded"
              ...
              = {totalUnidades} uds totales
            </span>
          )}
        </div>

        {calificado && (
          <span className="text-[9px] font-bold flex items-center gap-0.5 mt-1" style={{ color:"#15803d" }}>
      ...
      <div className="text-right shrink-0">
        <p className="text-sm font-black" style={{ color: C.purple }}>S/ {fmt(subtotal)}</p>
        <p className="text-[10px]" style={{ color: C.gray400 }}>S/ {fmt(precioUnit)} c/{esCaja ? "caja" : "ud"}</p>
        {!calificado && esEntregado && (
          <button onClick={onCalificar}
...
export default function MisPedidosPage() {
    ...
    const unsub = auth.onAuthStateChanged(user => {
      ...
      const unsubSnap = onSnapshot(q, snap => {
        const docs = snap.docs
          ...
          .sort((a: any, b: any) =>
            (b.fecha?.toDate?.()?.getTime() || 0) - (a.fecha?.toDate?.()?.getTime() || 0)
          );
        setPedidos(docs);
        generarNotifs(docs);
        setLoading(false);
  ...
  const generarNotifs = (ps: any[]) => {
    ...
    ps.forEach(p => {
      ...
      if (f && f > ayer)
        arr.push({ id:`n-${p.id}`, tipo:"nuevo", msg:`Pedido #${p.id.slice(0,8).toUpperCase()} registrado`, leida:false });
      if ((p.estado||"").toLowerCase() === "enviado")
        arr.push({ id:`e-${p.id}`, tipo:"envio", msg:`Pedido #${p.id.slice(0,8).toUpperCase()} despachado`, leida:false });
    });
    setNotifs(arr);
  };
  ...
  /* ── Totales de un pedido considerando caja/unidad ── */
  const getResumenItems = (items: any[]) => {
    let totalCajas = 0; let totalUnidades = 0; let totalPiezas = 0;
    (items || []).forEach(it => {
      ...
      if (esCaja)  { totalCajas += cant; totalUnidades += cant * uds; }
      else         { totalUnidades += cant; }
      totalPiezas++;
    });
    return { totalCajas, totalUnidades, totalPiezas };
  };
  ...
  const exportarExcel = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(pedidosFiltrados.map(p => {
      const igv = calcIGV(p.total||0);
      const res = getResumenItems(p.items);
      return {
        ...
        "Estado":          p.estado || "",
        "Total Cajas":     res.totalCajas,
        "Total Unidades":  res.totalUnidades,
  ...
  const generarPDF = (pedido: any) => {
    ...
    autoTable(d, {
      startY: 44,
      head: [["Producto","SKU","Tipo","Cant.","Uds/Caja","Total Uds","Precio","Subtotal"]],
      body: (pedido.items||[]).map((it: any) => {
        const esCaja   = it.tipoCompra==="caja" || it.unidad_venta?.toLowerCase().includes("caja");
        const udsCaja  = Number(it.unidadesPorCaja||it.unidades_por_caja)||10;
        const cant     = Number(it.cantidad)||1;
        const totalUds = esCaja ? cant * udsCaja : cant;
        return [it.nombre, it.sku||"—", esCaja?"Por Caja":"Por Unidad", cant, esCaja?udsCaja:"—", totalUds,
          `S/ ${fmt(it.precioBase||0)}`, `S/ ${fmt((it.precioBase||0)*cant)}`];
      }),
      theme: "grid",
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {pedidoSel.items?.map((it: any) => {
                      ...
                      return (
                        <button key={it.id} disabled={calif} onClick={() => setProdSel(it)}
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px]" style={{ color:C.gray500 }}>
                              {it.tipoCompra === "caja"
                                ? `${it.cantidad} caja${it.cantidad!==1?"s":""} × ${it.unidadesPorCaja||10} uds`
                ...
                <>
                  ...
                  <div>
                    ...
                    <textarea value={comentario} onChange={e => setComentario(e.target.value)}
                      placeholder="Comparte tu experiencia con este producto..." rows={4}
                  ...
                  <div>
                        ...
                        {previews.map((img, i) => (
                          <div key={i} className="relative aspect-square rounded-xl overflow-hidden group" style={{ background:C.gray100 }}>
                            ...
                            <button onClick={() => { setImagenesRes(p => p.filter((_,j)=>j!==i)); setPreviews(p => p.filter((_,j)=>j!==i)); }}
                              className="absolute top-1 right-1 w-5 h-5 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100"
                    ...
                    {previews.length < 4 && (
                      <label className="flex flex-col items-center justify-center border-2 border-dashed p-4 rounded-xl cursor-pointer"
                  ...
                  <button onClick={enviarResena} disabled={enviando || !comentario.trim()}
                    className="w-full py-3.5 rounded-xl font-black text-sm text-white flex items-center justify-center gap-2 disabled:opacity-50"
                    style={{ background:enviando||!comentario.trim()?C.gray300:`linear-gradient(135deg,${C.purple},${C.purpleDark})`, boxShadow:enviando||!comentario.trim()?"none":`0 4px 16px ${C.purple}40` }}>
                    {enviando ? <><RefreshCw size={15} className="animate-spin" />Enviando...</> : <><Send size={15} />Publicar opinión</>}
                  </button>
                </>
              )}
            </div>

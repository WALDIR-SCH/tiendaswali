...
interface Pedido    { id: string; numero?: string; cliente?: string; empresa?: string; total?: number; monto?: number; estado?: string; fecha?: any; items?: any[]; }
...
export default function AdminDashboard() {
  ...
  const stockCritico = productos
    .filter(p => (p.stock_cajas ?? 0) <= (p.stock_minimo_cajas ?? p.stockMinimo ?? 5))
    .slice(0, 5);
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:28 }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <Link href="/admin/pedidos/nuevo" className="btn-sec" style={{
            ...
            border:`1px solid ${C.purple}28`, fontSize:13, fontWeight:700, color:C.purple,
            textDecoration:"none",
          }}>
            + Pedido
          </Link>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 290px 290px", gap:14, marginBottom:22 }}>
        ...
        <div style={{ ...card, padding:"26px" }}>
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:20 }}>
            ...
            <Link href="/admin/pedidos" style={{ fontSize:12, fontWeight:700, color:C.purple, textDecoration:"none", display:"flex", alignItems:"center", gap:5, padding:"5px 14px", borderRadius:9, background:`${C.purple}10`, border:`1px solid ${C.purple}28` }}>
              Ver todos →
            </Link>
          </div>
          {loading ? (
            [...Array(4)].map((_,i) => (
              <div key={i} style={{ height:60, borderRadius:13, background:`${C.purple}06`, marginBottom:9, animation:"pulse 1.4s ease infinite" }} />
            ))
        ...
        <div style={{ ...card, padding:"26px" }}>
          ...
          {loading ? (
            [...Array(4)].map((_,i) => (
              <div key={i} style={{ height:50, borderRadius:12, background:`${C.orange}08`, marginBottom:9, animation:"pulse 1.4s ease infinite" }} />
            ))
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
              {stockCritico.map(p => {
                const s = p.stock_cajas ?? 0;
                const m = p.stock_minimo_cajas ?? p.stockMinimo ?? 5;
      ...
      {cotPend > 0 && (
        <div style={{ ...card, padding:"26px", marginBottom:22 }}>
          <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:20 }}>
            ...
            <Link href="/admin/cotizaciones" style={{ fontSize:12, fontWeight:700, color:C.purple, textDecoration:"none", padding:"5px 14px", borderRadius:9, background:`${C.purple}10`, border:`1px solid ${C.purple}28` }}>
              Ver todas →
            </Link>
          </div>

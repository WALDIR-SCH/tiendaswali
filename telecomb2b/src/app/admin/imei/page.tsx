...
import {
  ...
  ClipboardList, ListFilter, Hash, ShoppingBag, Bell, Sparkles,
} from "lucide-react";

/* ─── TIPOS ─── */
interface RegistroIMEI {
  id: string; imei: string; imei2?: string;
  productoId: string; productoNombre: string; sku: string;
  marca: string; modelo: string; color: string; almacenamiento: string;
  lote: string;
  estado: "disponible" | "vendido" | "reservado" | "garantia" | "defectuoso";
  pedidoId?: string; clienteNombre?: string;
  fechaIngreso: any; fechaVenta?: any;
  garantiaMeses: number; notas: string;
}

interface GrupoProducto {
  productoId: string; productoNombre: string; sku: string;
  marca: string; color: string; almacenamiento: string;
  disponibles: number; vendidos: number; reservados: number;
  garantia: number; defectuosos: number; total: number;
  lotes: string[]; imeis: RegistroIMEI[];
}

interface GrupoLote {
  lote: string; productoNombre: string; marca: string;
  total: number; disponibles: number; vendidos: number;
  imeis: RegistroIMEI[]; fechaIngreso: any;
}

interface Producto {
  id: string; nombre_producto?: string; nombre?: string; sku?: string;
  marca?: string; modelo?: string; color?: string;
  capacidad_almacenamiento?: string; precio_unitario?: number; garantia_meses?: number;
}

/* ─── PALETA ─── */
const C = {
  purple:"#7c3aed", purpleLight:"#8b5cf6", purpleBg:"#ede9fe", purpleBorder:"#ddd6fe",
  green:"#28FB4B", greenDark:"#16a34a", greenBg:"#f0fdf4", greenBorder:"#bbf7d0",
  yellow:"#F6FA00", orange:"#FF6600", orangeBg:"#fff7ed", orangeBorder:"#fed7aa",
...
function ModalDetalleProducto({ grupo, onClose, onCambiarEstado }: {
  grupo: GrupoProducto; onClose: ()=>void;
  ...
  const copiar = (imei:string) => { navigator.clipboard.writeText(imei); setCopiado(imei); setTimeout(()=>setCopiado(null),1500); };
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 24px",borderBottom:`1px solid ${C.gray100}` }}>
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            ...
            <div>
              ...
              <p style={{ margin:0,fontSize:12,color:C.gray500 }}>SKU: {grupo.sku} · {grupo.marca} · {grupo.color} {grupo.almacenamiento} · {grupo.total} IMEIs</p>
            </div>
          </div>
        ...
        <div style={{ padding:"20px 24px",display:"flex",flexDirection:"column",gap:16 }}>
          <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
            {[
              ...
              { label:"Total",      val:grupo.total,      bg:C.gray50,   color:C.gray900,  border:C.gray200      },
            ].filter(s=>s.val>0).map(s=>(
              <div key={s.label} style={{ display:"flex",alignItems:"center",gap:6,padding:"6px 12px",borderRadius:20,border:`1.5px solid ${s.border}`,background:s.bg }}>
          ...
          <div style={{ display:"flex",gap:10 }}>
            <div style={{ flex:1,position:"relative" }}>
              ...
              <input value={busqueda} onChange={e=>setBusqueda(e.target.value)} placeholder="Filtrar por IMEI..."
            ...
            <table style={{ width:"100%",borderCollapse:"collapse" }}>
              <thead style={{ position:"sticky",top:0 }}>
                <tr style={{ background:C.gray50,borderBottom:`1px solid ${C.gray200}` }}>
                  {["#","IMEI 1","IMEI 2","Lote","Estado","Cliente","Ingreso","Acción"].map(h=>(
                    <th key={h} style={{ padding:"9px 12px",fontSize:10,fontWeight:800,color:C.gray500,textAlign:"left",textTransform:"uppercase" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtrados.length===0 ? (
                  ...
                  <tr key={r.id} style={{ borderBottom:`1px solid ${C.gray100}` }}
                    ...
                    <td style={{ padding:"9px 12px",fontSize:11,color:C.gray500 }}>{i+1}</td>
                    <td style={{ padding:"9px 12px" }}>
                      <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                        <span style={{ fontFamily:"monospace",fontSize:12,fontWeight:700,color:C.gray900 }}>{r.imei}</span>
                        <button onClick={()=>copiar(r.imei)} style={{ width:22,height:22,borderRadius:6,border:`1px solid ${C.gray200}`,background:C.white,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>
                          {copiado===r.imei ? <Check size={10} style={{ color:C.greenDark }}/> : <Copy size={10} style={{ color:C.gray500 }}/>}
                        </button>
                      </div>
                    </td>
                    <td style={{ padding:"9px 12px",fontFamily:"monospace",fontSize:11,color:C.gray500 }}>{r.imei2||"—"}</td>
                    <td style={{ padding:"9px 12px" }}><span style={{ fontSize:10,fontWeight:700,padding:"2px 6px",borderRadius:6,background:C.purpleBg,color:C.purple }}>{r.lote}</span></td>
                    <td style={{ padding:"9px 12px" }}><EstadoBadge estado={r.estado} /></td>
                    <td style={{ padding:"9px 12px",fontSize:11,color:r.clienteNombre?C.gray900:C.gray300 }}>{r.clienteNombre||"—"}</td>
                    <td style={{ padding:"9px 12px",fontSize:11,color:C.gray500 }}>{fmtFecha(r.fechaIngreso)}</td>
                    <td style={{ padding:"9px 12px" }}>
...
/* ══════════════════════════════════════
   ...
   — Con notificación automática al cliente
══════════════════════════════════════ */
interface PedidoResumen {
  id: string; numero: string; clienteNombre: string; clienteEmail: string;
  clienteTelefono: string; clienteRut: string; empresa: string; direccion: string;
  estado: string; total: number; itemProducto: any|null;
  tipoCompra: "caja"|"unidad"|null;
  cantidadCajas: number; cantidadUnidades: number; unidadesPorCaja: number; items: any[];
  clienteId?: string;
}

function ModalAsignar({ grupo, onClose, onAsignar }: {
  grupo: GrupoProducto; onClose: ()=>void;
  onAsignar: (ids:string[], pedidoId:string, clienteNombre:string, pedido:PedidoResumen)=>void;
}) {
    ...
    const unsub = onSnapshot(q, snap => {
      const docs = snap.docs.map(d => {
        ...
        const itemProducto = items.find((it:any) =>
          it.productoId===grupo.productoId || it.id===grupo.productoId ||
          it.idOriginal===grupo.productoId || (it.sku && it.sku===grupo.sku)
        ) || null;
        const tipoCompra: "caja"|"unidad"|null = itemProducto
          ? (itemProducto.tipoCompra || (itemProducto.id?.includes("-caja") || itemProducto.unidad_venta?.toLowerCase?.().includes("caja") ? "caja" : "unidad"))
          : null;
        const udsPorCajaItem = Number(itemProducto?.unidadesPorCaja||itemProducto?.unidades_por_caja)||0;
        ...
        const cantidadUnidades = tipoCompra==="caja" ? cantPedida*udsPorCaja : cantPedida;
        return {
          id: d.id,
          ...
          itemProducto, tipoCompra, cantidadCajas, cantidadUnidades, unidadesPorCaja:udsPorCaja, items,
  ...
  const pedidosFiltrados = useMemo(() => {
    ...
    return pedidosDB.filter(p =>
      p.numero.toLowerCase().includes(t)||p.clienteNombre.toLowerCase().includes(t)||
      p.empresa.toLowerCase().includes(t)||p.clienteEmail.toLowerCase().includes(t)||
      p.clienteRut.includes(t)||p.id.toLowerCase().includes(t)
    ).slice(0,8);
  }, [pedidosDB, busquedaPedido]);

  const seleccionarPedido = (p: PedidoResumen) => {
    setPedidoSel(p); setBusquedaPedido(p.numero); setShowDropdown(false);
    const imeisReq = p.cantidadUnidades>0 ? p.cantidadUnidades : (p.cantidadCajas||1);
    const cantAuto = Math.min(Math.max(imeisReq,1), disponibles.length);
    setCantidad(cantAuto);
    if (modoAuto) setSeleccionados(disponibles.slice(0,cantAuto).map(r=>r.id));
  };

  useEffect(() => {
    if (modoAuto) setSeleccionados(disponibles.slice(0,Math.min(cantidad,disponibles.length)).map(r=>r.id));
  }, [cantidad,modoAuto]);

  const handleAsignar = async () => {
    if (seleccionados.length===0) { alert("No hay IMEIs seleccionados"); return; }
    if (!pedidoSel) { alert("Selecciona un pedido"); return; }
    setSaving(true);
    try {
      const nombre = pedidoSel.empresa||pedidoSel.clienteNombre||pedidoSel.clienteEmail;
      await onAsignar(seleccionados, pedidoSel.id, nombre, pedidoSel);
      onClose();
      ...
      <div style={{ background:C.white,borderRadius:20,width:"100%",maxWidth:560,maxHeight:"92vh",overflowY:"auto",border:`1px solid ${C.gray200}`,boxShadow:"0 24px 80px rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ padding:"20px 24px",display:"flex",flexDirection:"column",gap:16 }}>
          ...
          <div style={{ padding:"12px 16px",borderRadius:12,background:C.greenBg,border:`1px solid ${C.greenBorder}`,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
            <span style={{ fontSize:13,color:C.greenDark,fontWeight:600 }}>IMEIs disponibles</span>
            <span style={{ fontSize:20,fontWeight:900,color:C.greenDark }}>{disponibles.length}</span>
          </div>

          {/* Info notificación automática */}
          <div style={{ display:"flex",alignItems:"flex-start",gap:8,padding:"10px 14px",borderRadius:12,background:`${C.purple}08`,border:`1px solid ${C.purpleBorder}` }}>
            <Bell size={14} style={{ color:C.purple,flexShrink:0,marginTop:1 }} />
            <p style={{ margin:0,fontSize:12,color:C.purple,fontWeight:600,lineHeight:1.5 }}>
              Al asignar, el cliente recibirá una notificación automática con los IMEIs registrados en OSIPTEL.
            </p>
          </div>

          {/* Buscador pedidos */}
          <div>
            <label style={{ fontSize:12,fontWeight:700,color:C.gray700,display:"block",marginBottom:6 }}>Buscar pedido *</label>
            <div style={{ position:"relative" }}>
              <Search size={14} style={{ position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:C.gray300,pointerEvents:"none" }} />
              <input value={busquedaPedido}
                onChange={e=>{ setBusquedaPedido(e.target.value); setPedidoSel(null); setShowDropdown(true); }}
                onFocus={()=>setShowDropdown(true)}
                placeholder="N° pedido, cliente, empresa o email..."
                style={{ width:"100%",paddingLeft:36,paddingRight:pedidoSel?36:12,paddingTop:10,paddingBottom:10,borderRadius:11,border:`1.5px solid ${pedidoSel?C.greenDark:C.gray200}`,fontSize:13,outline:"none",color:C.gray900 }} />
              {pedidoSel && <CheckCircle size={16} style={{ position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",color:C.greenDark }} />}
              {showDropdown && !pedidoSel && (
                <div style={{ position:"absolute",top:"calc(100% + 4px)",left:0,right:0,background:C.white,borderRadius:14,border:`1.5px solid ${C.gray200}`,boxShadow:"0 12px 40px rgba(0,0,0,0.12)",zIndex:20,overflow:"hidden",maxHeight:260,overflowY:"auto" }}>
                  {buscando ? <div style={{ padding:"16px",textAlign:"center",fontSize:12,color:C.gray500 }}>Cargando...</div>
                  : pedidosFiltrados.length===0 ? <div style={{ padding:"16px",textAlign:"center",fontSize:12,color:C.gray500 }}>Sin resultados</div>
                  : pedidosFiltrados.map(p=>(
                    <button key={p.id} onClick={()=>seleccionarPedido(p)}
                      style={{ width:"100%",display:"flex",alignItems:"flex-start",gap:10,padding:"10px 14px",border:"none",background:"transparent",cursor:"pointer",textAlign:"left",borderBottom:`1px solid ${C.gray100}` }}
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:2 }}>
                          ...
                          <span style={{ fontSize:9,fontWeight:700,padding:"1px 6px",borderRadius:20,background:C.purpleBg,color:C.purple }}>{p.estado}</span>
                        </div>
                        <p style={{ margin:0,fontSize:12,color:C.gray700,fontWeight:600 }}>{p.empresa||p.clienteNombre||p.clienteEmail}</p>
                        {p.clienteRut && <p style={{ margin:"1px 0 0",fontSize:10,color:C.gray500 }}>RUC: {p.clienteRut}</p>}
                      </div>
                      {p.itemProducto && (
                        <div style={{ flexShrink:0,textAlign:"right" }}>
                          <p style={{ margin:0,fontSize:11,fontWeight:800,color:C.purple }}>
                            {p.tipoCompra==="caja" ? `${p.cantidadCajas} caja(s)` : `${p.cantidadUnidades} uds`}
                          </p>
                          <p style={{ margin:0,fontSize:9,color:C.gray500 }}>→ {p.cantidadUnidades} IMEIs</p>
                        </div>
                      )}
                    </button>
                  ))}
            ...
            <div style={{ borderRadius:14,border:`2px solid ${C.greenBorder}`,overflow:"hidden" }}>
              <div style={{ padding:"10px 16px",background:C.greenBg,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                  ...
                  <span style={{ fontSize:13,fontWeight:800,color:C.greenDark }}>Pedido: {pedidoSel.numero}</span>
                </div>
                <button onClick={()=>{ setPedidoSel(null); setBusquedaPedido(""); }} style={{ width:24,height:24,borderRadius:6,border:`1px solid ${C.greenBorder}`,background:C.white,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>
              ...
              <div style={{ padding:"14px 16px",background:"#fafffe" }}>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
                  {[
                    ["Empresa", pedidoSel.empresa||pedidoSel.clienteNombre],
                    ["RUC/DNI", pedidoSel.clienteRut||"—"],
                    ["Email", pedidoSel.clienteEmail||"—"],
                    ["Teléfono", pedidoSel.clienteTelefono||"—"],
                  ].map(([k,v])=>(
                    <div key={k as string}>
                      <p style={{ margin:0,fontSize:9,fontWeight:700,color:C.gray500,textTransform:"uppercase" }}>{k}</p>
                      <p style={{ margin:"1px 0 0",fontSize:12,fontWeight:600,color:C.gray900 }}>{v||"—"}</p>
                    </div>
                  ))}
                </div>
                    ...
                    {pedidoSel.tipoCompra==="caja" ? (
                      <>
                        <div style={{ padding:"8px 14px",borderRadius:10,background:C.orangeBg,border:`1px solid ${C.orangeBorder}`,textAlign:"center" }}>
                          ...
                          <p style={{ margin:0,fontSize:10,fontWeight:700,color:C.orange }}>caja(s)</p>
                        </div>
            ...
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6 }}>
              ...
              {pedidoSel && pedidoSel.cantidadUnidades>0 && cantidad!==pedidoSel.cantidadUnidades && (
                <button onClick={()=>setCantidad(Math.min(pedidoSel.cantidadUnidades,disponibles.length))}
                  ...
                  Usar del pedido ({pedidoSel.cantidadUnidades})
                </button>
              )}
            </div>
            <div style={{ display:"flex",alignItems:"center",gap:10 }}>
              <button onClick={()=>setCantidad(c=>Math.max(1,c-1))} disabled={cantidad<=1}
                style={{ width:38,height:38,borderRadius:10,border:`1.5px solid ${C.gray200}`,background:C.gray100,fontSize:18,cursor:cantidad<=1?"not-allowed":"pointer",color:cantidad<=1?C.gray300:C.gray700,display:"flex",alignItems:"center",justifyContent:"center" }}>−</button>
              ...
              <button onClick={()=>setCantidad(c=>Math.min(disponibles.length,c+1))} disabled={cantidad>=disponibles.length}
                style={{ width:38,height:38,borderRadius:10,border:`1.5px solid ${C.gray200}`,background:C.gray100,fontSize:18,cursor:cantidad>=disponibles.length?"not-allowed":"pointer",color:cantidad>=disponibles.length?C.gray300:C.gray700,display:"flex",alignItems:"center",justifyContent:"center" }}>+</button>
              <span style={{ fontSize:12,color:C.gray500 }}>de <strong style={{ color:C.greenDark }}>{disponibles.length}</strong> disponibles</span>
            </div>
          </div>

          {/* Modo selección */}
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
            {[["auto","🤖 Automático","El sistema elige"],["manual","✋ Manual","Elige tú cuáles"]].map(([modo,titulo,desc])=>(
          ...
          {!modoAuto ? (
            <div>
              <p style={{ fontSize:12,fontWeight:700,color:C.gray700,marginBottom:8 }}>
                Selecciona ({seleccionados.length} de {cantidad})
              </p>
              <div style={{ border:`1px solid ${C.gray200}`,borderRadius:12,maxHeight:200,overflowY:"auto" }}>
                {disponibles.map(r=>{
                  const sel = seleccionados.includes(r.id);
                  return (
                    <div key={r.id} onClick={()=>{ if(sel) setSeleccionados(p=>p.filter(id=>id!==r.id)); else if(seleccionados.length<cantidad) setSeleccionados(p=>[...p,r.id]); }}
                      style={{ display:"flex",alignItems:"center",gap:12,padding:"10px 14px",cursor:"pointer",background:sel?`${C.purple}08`:C.white,borderBottom:`1px solid ${C.gray100}` }}>
                      <div style={{ width:20,height:20,borderRadius:6,border:`2px solid ${sel?C.purple:C.gray300}`,background:sel?C.purple:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                      ...
                      <span style={{ fontSize:10,color:C.gray500 }}>{r.lote}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ padding:"12px 16px",borderRadius:12,background:C.purpleBg,border:`1px solid ${C.purpleBorder}` }}>
              <p style={{ margin:0,fontSize:13,color:C.purple,fontWeight:600 }}>
                Se asignarán automáticamente <strong>{Math.min(cantidad,disponibles.length)}</strong> IMEIs disponibles
              </p>
              <div style={{ marginTop:8,display:"flex",flexWrap:"wrap",gap:5 }}>
                {disponibles.slice(0,Math.min(cantidad,5)).map(r=>(
                  <span key={r.id} style={{ fontFamily:"monospace",fontSize:10,fontWeight:700,color:C.purple,padding:"2px 7px",borderRadius:6,background:C.white,border:`1px solid ${C.purpleBorder}` }}>{r.imei}</span>
                ))}
                {cantidad>5 && <span style={{ fontSize:10,color:C.gray500,padding:"2px 7px",borderRadius:6,background:C.white,border:`1px solid ${C.gray200}` }}>+{Math.min(cantidad,disponibles.length)-5} más</span>}
              </div>
            </div>
          )}
          ...
          <div style={{ display:"flex",gap:10 }}>
            ...
            <button onClick={handleAsignar} disabled={saving||seleccionados.length===0||!pedidoSel}
              style={{ flex:2,padding:"11px",borderRadius:11,background:`linear-gradient(135deg,${C.purple},${C.purpleLight})`,color:C.white,border:"none",fontSize:13,fontWeight:800,cursor:"pointer",opacity:saving||seleccionados.length===0||!pedidoSel?0.5:1,boxShadow:`0 4px 14px ${C.purple}40`,display:"flex",alignItems:"center",justifyContent:"center",gap:6 }}>
              {saving ? "Asignando..." : <><Sparkles size={14}/> Asignar {seleccionados.length} IMEI{seleccionados.length!==1?"s":""} + Notificar</>}
            </button>
          </div>
        </div>
      </div>
...
function ModalRegistrar({ onClose, onSaved }: { onClose:()=>void; onSaved:()=>void }) {
  ...
  const [form, setForm] = useState({ imei:"",imei2:"",productoId:"",productoNombre:"",sku:"",marca:"",modelo:"",color:"",almacenamiento:"", lote:`LOTE-${new Date().toISOString().slice(0,7)}`,garantiaMeses:12,notas:"" });

  useEffect(() => { getDocs(collection(db,"productos")).then(snap=>setProductos(snap.docs.map(d=>({id:d.id,...d.data()} as Producto)))); }, []);

  const validate = (s:string) => /^\d{15}$/.test(s.replace(/\s/g,""));

  const selectProd = (p:Producto) => setForm(f=>({ ...f,productoId:p.id,productoNombre:p.nombre_producto||p.nombre||"",sku:p.sku||"",marca:p.marca||"",modelo:p.modelo||"",color:p.color||"",almacenamiento:p.capacidad_almacenamiento||"",garantiaMeses:p.garantia_meses||12 }));

  const parseLote = () => loteTexto.split("\n").map(l=>l.trim()).filter(Boolean).map(l=>{ const [i1,i2]=l.split(",").map(x=>x.trim()); return {imei:i1||"",imei2:i2||"",valido:validate(i1||"")}; });

  const handlePreview = () => { if(!form.productoId){alert("Selecciona producto");return;} const p=parseLote(); if(p.length===0){alert("Sin IMEIs");return;} setPreview(p); setStep(2); };

  const handleSave = async () => {
    if(!form.productoId){alert("Selecciona producto");return;}
    setSaving(true);
    try {
      if(modo==="uno") {
        ...
        validos.forEach(p=>{ const ref=doc(collection(db,"imeis")); batch.set(ref,{...form,imei:p.imei,imei2:p.imei2,estado:"disponible",fechaIngreso:serverTimestamp()}); });
        await batch.commit();
      }
      onSaved(); onClose();
      ...
      <div style={{ background:C.white,borderRadius:20,width:"100%",maxWidth:580,border:`1px solid ${C.gray200}`,boxShadow:"0 24px 80px rgba(0,0,0,0.2)",marginTop:20 }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 24px",borderBottom:`1px solid ${C.gray100}` }}>
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            ...
            <div>
              <h3 style={{ margin:0,fontSize:15,fontWeight:800,color:C.gray900 }}>{step===1?"Registrar IMEIs":`Vista previa — ${preview.length} IMEIs`}</h3>
              <p style={{ margin:0,fontSize:11,color:C.gray500 }}>{step===2?`${validCount} válidos · ${invalidCount} inválidos`:"Ingreso de inventario"}</p>
            </div>
          </div>
          <button onClick={onClose} style={{ width:32,height:32,borderRadius:9,border:`1px solid ${C.gray200}`,background:C.white,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>
            <X size={15} style={{ color:C.gray500 }} />
        ...
        {step===1 ? (
          <div style={{ padding:"20px 24px",display:"flex",flexDirection:"column",gap:14 }}>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8 }}>
              {([["uno","📱 Individual"],["lote","📦 Por lote"]] as const).map(([m,l])=>(
                <button key={m} onClick={()=>setModo(m)} style={{ padding:"10px",borderRadius:11,border:`2px solid ${modo===m?C.purple:C.gray200}`,background:modo===m?C.purpleBg:C.white,fontSize:13,fontWeight:700,color:modo===m?C.purple:C.gray700,cursor:"pointer" }}>{l}</button>
              ))}
            </div>
            <div>
              <label style={{ fontSize:12,fontWeight:700,color:C.gray700,display:"block",marginBottom:6 }}>Modelo *</label>
            ...
            {modo==="uno" ? (
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12 }}>
                <div>
                  ...
                  <input value={form.imei} onChange={e=>setForm(f=>({...f,imei:e.target.value.replace(/\D/g,"").slice(0,15)}))} placeholder="353456789012345"
                ...
                <div>
                  ...
                  <input value={form.imei2} onChange={e=>setForm(f=>({...f,imei2:e.target.value.replace(/\D/g,"").slice(0,15)}))} placeholder="Opcional"
            ...
            <div style={{ display:"flex",gap:10 }}>
              <button onClick={onClose} style={{ flex:1,padding:"11px",borderRadius:11,border:`1px solid ${C.gray200}`,background:C.white,fontSize:13,fontWeight:600,color:C.gray700,cursor:"pointer" }}>Cancelar</button>
              {modo==="lote"
                ? <button onClick={handlePreview} style={{ flex:2,padding:"11px",borderRadius:11,background:`linear-gradient(135deg,${C.purple},${C.purpleLight})`,color:C.white,border:"none",fontSize:13,fontWeight:800,cursor:"pointer" }}>Vista previa →</button>
          ...
          <div style={{ padding:"20px 24px",display:"flex",flexDirection:"column",gap:14 }}>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8 }}>
              {[{l:"Total",v:preview.length,bg:C.purpleBg,c:C.purple},{l:"✓ Válidos",v:validCount,bg:C.greenBg,c:C.greenDark},{l:"⚠ Inválidos",v:invalidCount,bg:C.redBg,c:C.red}].map(s=>(
                <div key={s.l} style={{ padding:"10px 12px",borderRadius:10,background:s.bg,textAlign:"center" }}>
            ...
            <div style={{ border:`1px solid ${C.gray200}`,borderRadius:12,overflow:"hidden",maxHeight:260,overflowY:"auto" }}>
              <table style={{ width:"100%",borderCollapse:"collapse" }}>
                <thead><tr style={{ background:C.gray50 }}>{["#","IMEI 1","IMEI 2",""].map(h=><th key={h} style={{ padding:"8px 12px",fontSize:10,fontWeight:700,color:C.gray500,textAlign:"left" }}>{h}</th>)}</tr></thead>
                <tbody>
                  {preview.map((p,i)=>(
                    <tr key={i} style={{ borderBottom:`1px solid ${C.gray100}`,background:!p.valido?C.redBg:C.white }}>
                      <td style={{ padding:"7px 12px",fontSize:11,color:C.gray500 }}>{i+1}</td>
                      <td style={{ padding:"7px 12px",fontFamily:"monospace",fontSize:12,fontWeight:600,color:p.valido?C.gray900:C.red }}>{p.imei}</td>
                      <td style={{ padding:"7px 12px",fontFamily:"monospace",fontSize:11,color:C.gray500 }}>{p.imei2||"—"}</td>
                      <td style={{ padding:"7px 12px" }}>{p.valido?<CheckCircle size={14} style={{ color:C.greenDark }}/>:<span style={{ fontSize:9,fontWeight:700,color:C.red }}>Inválido</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            {invalidCount>0&&<div style={{ padding:"10px 14px",borderRadius:10,background:C.redBg,border:`1px solid ${C.redBorder}`,display:"flex",alignItems:"center",gap:8 }}><AlertTriangle size={14} style={{ color:C.red }}/><p style={{ margin:0,fontSize:12,color:C.red }}><strong>{invalidCount} inválido(s)</strong> serán ignorados.</p></div>}
            <div style={{ display:"flex",gap:10 }}>
              ...
              <button onClick={handleSave} disabled={saving||validCount===0} style={{ flex:2,padding:"11px",borderRadius:11,background:validCount===0?C.gray200:`linear-gradient(135deg,${C.purple},${C.purpleLight})`,color:validCount===0?C.gray500:C.white,border:"none",fontSize:13,fontWeight:800,cursor:validCount===0?"not-allowed":"pointer" }}>
...
export default function IMEIAdmin() {
  ...
  const [toast,         setToast]         = useState<{msg:string;tipo:"ok"|"info"} | null>(null);

  // ── Notificaciones de pedidos nuevos en tiempo real ──
  const [pedidosNuevos, setPedidosNuevos] = useState<any[]>([]);
  const [showNotifPanel, setShowNotifPanel] = useState(false);
  const [pedidosLeidos, setPedidosLeidos] = useState<Set<string>>(new Set());

  useEffect(() => {
    // Escuchar pedidos recientes (últimas 48h) para notificar ventas
    const hace48h = new Date(); hace48h.setHours(hace48h.getHours() - 48);
    const q = query(collection(db,"pedidos"), orderBy("fecha","desc"));
    const unsub = onSnapshot(q, snap => {
      const docs = snap.docs.map(d=>({id:d.id,...d.data()})).filter((p:any)=>{
        const f = p.fecha?.toDate?.();
        return f && f > hace48h;
      });
      setPedidosNuevos(docs);
    });
    return ()=>unsub();
  }, []);

  const pedidosSinLeer = pedidosNuevos.filter((p:any) => !pedidosLeidos.has(p.id));
  const marcarPedidoLeido = (id:string) => setPedidosLeidos(prev=>new Set([...prev,id]));

  const showToast = (msg:string, tipo:"ok"|"info"="ok") => {
    setToast({msg,tipo});
  ...
  // ── ASIGNAR IMEIs: actualiza IMEIs + pedido + notifica al cliente ──
  const asignarAPedido = useCallback(async (ids:string[], pedidoId:string, clienteNombre:string, pedido:PedidoResumen) => {
    const batch = writeBatch(db);

    // 1. Marcar IMEIs como vendidos
    const imeisNums: string[] = [];
    ids.forEach(id => {
      const registro = registros.find(r=>r.id===id);
      if (registro) imeisNums.push(registro.imei);
    ...
    // 2. Actualizar el pedido con los IMEIs asignados
    const pedidoRef = doc(db,"pedidos",pedidoId);
    batch.update(pedidoRef, {
      imeisAsignados: imeisNums,
      imeisAsignadosIds: ids,
      estadoImeis: "asignados",
      fechaAsignacionImeis: serverTimestamp(),
    });

    await batch.commit();

    // 3. Crear notificación para el cliente en Firestore
    try {
      const esCaja = pedido.tipoCompra === "caja";
      const resumenCompra = esCaja
        ? `${pedido.cantidadCajas} caja${pedido.cantidadCajas!==1?"s":""} × ${pedido.unidadesPorCaja} uds = ${pedido.cantidadUnidades} equipos`
        : `${pedido.cantidadUnidades} unidad${pedido.cantidadUnidades!==1?"es":""}`;

      // Lista de IMEIs formateada para el mensaje
      const listaImeis = imeisNums.slice(0,5).join(", ") + (imeisNums.length>5 ? ` y ${imeisNums.length-5} más` : "");

      await addDoc(collection(db,"notificaciones"), {
        tipo:           "imeis_asignados",
        pedidoId,
        clienteEmail:   pedido.clienteEmail,
        clienteId:      pedido.clienteId || "",
        titulo:         "📱 IMEIs registrados en OSIPTEL",
        mensaje:        `Tu pedido ${pedido.numero} (${resumenCompra}) tiene ${ids.length} IMEI${ids.length!==1?"s":""} asignados y registrados. Equipos: ${listaImeis}.`,
        imeisAsignados: imeisNums,
        cantidadImeis:  ids.length,
        productoNombre: gruposPorProducto.find(g=>g.productoId===asignarProd?.productoId)?.productoNombre || "",
        tipoCompra:     pedido.tipoCompra,
        cantidadCajas:  pedido.cantidadCajas,
        cantidadUnidades: pedido.cantidadUnidades,
        unidadesPorCaja: pedido.unidadesPorCaja,
        leida:          false,
        timestamp:      serverTimestamp(),
      });
    } catch (e) {
      console.warn("Notificación fallida (no crítico):", e);
    }

    showToast(`✅ ${ids.length} IMEI${ids.length!==1?"s":""} asignados y cliente notificado`, "ok");
  ...
  const gruposPorProducto = useMemo(():GrupoProducto[]=> {
    ...
    registros.forEach(r=>{
      ...
      if(!mapa.has(key)) mapa.set(key,{productoId:r.productoId,productoNombre:r.productoNombre,sku:r.sku,marca:r.marca,color:r.color,almacenamiento:r.almacenamiento,disponibles:0,vendidos:0,reservados:0,garantia:0,defectuosos:0,total:0,lotes:[],imeis:[]});
      const g=mapa.get(key)!;
      g.total++; g.imeis.push(r);
  ...
  const gruposPorLote = useMemo(():GrupoLote[]=> {
    ...
    registros.forEach(r=>{
      const key=r.lote;
      if(!mapa.has(key)) mapa.set(key,{lote:r.lote,productoNombre:r.productoNombre,marca:r.marca,total:0,disponibles:0,vendidos:0,imeis:[],fechaIngreso:r.fechaIngreso});
      const g=mapa.get(key)!;
      g.total++; g.imeis.push(r);
      if(r.estado==="disponible") g.disponibles++;
      if(r.estado==="vendido")    g.vendidos++;
    });
    return Array.from(mapa.values()).sort((a,b)=>{ try{return (b.fechaIngreso?.toDate?.()?.getTime()||0)-(a.fechaIngreso?.toDate?.()?.getTime()||0);}catch{return 0;} });
  }, [registros]);
  ...
  const toggleLote = (lote:string) => {
    setExpandedLotes(prev=>{ const n=new Set(prev); n.has(lote)?n.delete(lote):n.add(lote); return n; });
  };
  ...
  return (
    <div style={{ fontFamily:"'Plus Jakarta Sans', system-ui, sans-serif",minHeight:"100vh",background:C.gray50 }} className="dash-in">
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position:"fixed",top:20,right:20,zIndex:90,padding:"12px 20px",borderRadius:14,background:toast.tipo==="ok"?C.greenDark:`${C.purple}`,color:C.white,fontSize:13,fontWeight:700,boxShadow:"0 8px 30px rgba(0,0,0,0.25)",animation:"slideIn .3s ease",display:"flex",alignItems:"center",gap:8 }}>
      ...
      <div style={{ background:C.white,borderBottom:`1px solid ${C.gray200}`,padding:"20px 24px",display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:12 }}>
        <div>
          ...
          <p style={{ fontSize:13,color:C.gray500,margin:"3px 0 0" }}>
            {stats.modelos} modelos · {stats.lotes} lotes · {stats.total.toLocaleString()} equipos
          </p>
        </div>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          {/* Campana pedidos nuevos */}
          <div style={{ position:"relative" }}>
            <button onClick={()=>setShowNotifPanel(v=>!v)} style={{
              position:"relative",width:44,height:44,borderRadius:12,
              border:`1.5px solid ${pedidosSinLeer.length>0?C.purple:C.gray200}`,
              background:pedidosSinLeer.length>0?`${C.purple}08`:C.white,
              display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",
            }}>
              <Bell size={18} style={{ color:pedidosSinLeer.length>0?C.purple:C.gray500 }} />
              {pedidosSinLeer.length>0&&(
                <span style={{ position:"absolute",top:-5,right:-5,minWidth:18,height:18,background:C.orange,color:"#fff",borderRadius:20,fontSize:10,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 4px",animation:"bell-pulse 2s ease-in-out infinite" }}>
                  {pedidosSinLeer.length}
                </span>
              )}
            </button>
            {showNotifPanel && (
              <>
                ...
                <div style={{ position:"absolute",top:"calc(100% + 8px)",right:0,width:380,background:C.white,borderRadius:16,border:`1px solid ${C.gray200}`,boxShadow:"0 20px 60px rgba(0,0,0,0.12)",zIndex:50,overflow:"hidden" }}>
                  <div style={{ padding:"14px 18px",borderBottom:`1px solid ${C.gray100}`,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                    <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                      ...
                      <span style={{ fontSize:13,fontWeight:800,color:C.gray900 }}>Pedidos recientes</span>
                      {pedidosSinLeer.length>0&&<span style={{ fontSize:10,fontWeight:800,padding:"2px 7px",borderRadius:20,background:C.orange,color:"#fff" }}>{pedidosSinLeer.length} sin leer</span>}
                    </div>
                    <button onClick={()=>setShowNotifPanel(false)} style={{ background:"none",border:"none",cursor:"pointer",color:C.gray500 }}><X size={14}/></button>
                  </div>
                  <div style={{ maxHeight:340,overflowY:"auto" }}>
                    {pedidosNuevos.length===0 ? (
                      <div style={{ padding:"32px",textAlign:"center",color:C.gray500 }}>
                        ...
                        <p style={{ margin:0,fontSize:13 }}>Sin pedidos recientes</p>
                      </div>
                    ) : pedidosNuevos.map((p:any)=>{
                      const leido = pedidosLeidos.has(p.id);
                      const items: any[] = p.items||[];
                      const totalUds = items.reduce((s:number,it:any)=>{
                        const esCaja = it.tipoCompra==="caja"||it.unidad_venta?.toLowerCase?.().includes("caja");
                        const cant = Number(it.cantidad)||1;
                        const uds  = Number(it.unidadesPorCaja||it.unidades_por_caja)||10;
                        return s + (esCaja ? cant*uds : cant);
                      },0);
                      const totalCajas = items.reduce((s:number,it:any)=>{
                        const esCaja = it.tipoCompra==="caja"||it.unidad_venta?.toLowerCase?.().includes("caja");
                        return s + (esCaja ? Number(it.cantidad)||0 : 0);
                      },0);
                      const yaAsignado = p.imeisAsignados?.length>0;
                      // Buscar grupo de producto
                      const grupoMatch = gruposPorProducto.find(g=> items.some((it:any)=>it.productoId===g.productoId||it.id===g.productoId||(it.sku&&it.sku===g.sku)));

                      return (
                        <div key={p.id} style={{ padding:"14px 18px",borderBottom:`1px solid ${C.gray100}`,background:leido?C.white:`${C.purple}04`,cursor:"pointer" }}
                          onClick={()=>marcarPedidoLeido(p.id)}
                          onMouseEnter={e=>(e.currentTarget as HTMLDivElement).style.background=`${C.purple}08`}
                          onMouseLeave={e=>(e.currentTarget as HTMLDivElement).style.background=leido?C.white:`${C.purple}04`}>
                          <div style={{ display:"flex",alignItems:"flex-start",gap:10 }}>
                            <div style={{ width:36,height:36,borderRadius:10,background:yaAsignado?C.greenBg:`${C.orange}12`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                              {yaAsignado ? <CheckCircle size={16} style={{ color:C.greenDark }}/> : <ShoppingBag size={16} style={{ color:C.orange }}/>}
                            </div>
                            <div style={{ flex:1,minWidth:0 }}>
                              <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:3 }}>
                                <span style={{ fontSize:13,fontWeight:800,color:C.purple }}>{p.numero||`#${p.id.slice(-8).toUpperCase()}`}</span>
                                {yaAsignado
                                  ? <span style={{ fontSize:9,fontWeight:700,padding:"1px 7px",borderRadius:20,background:C.greenBg,color:C.greenDark,border:`1px solid ${C.greenBorder}` }}>✓ IMEIs asignados</span>
                                  : <span style={{ fontSize:9,fontWeight:700,padding:"1px 7px",borderRadius:20,background:`${C.orange}15`,color:C.orange,border:`1px solid ${C.orange}30` }}>⚡ Requiere IMEIs</span>}
                              </div>
                              <p style={{ margin:0,fontSize:12,color:C.gray700,fontWeight:600 }}>{p.clienteNombre||p.clienteEmail||p.empresa||"—"}</p>
                              <p style={{ margin:"2px 0 0",fontSize:11,color:C.gray500 }}>
                                {totalCajas>0 ? `${totalCajas} caja(s) · ` : ""}{totalUds} equipo(s) · {fmtFecha(p.fecha)}
                              </p>
                            </div>
                            {!leido && <div style={{ width:7,height:7,borderRadius:"50%",background:C.purple,flexShrink:0,marginTop:4 }} />}
                          </div>
                          {/* Botón asignar IMEI si no está asignado y hay grupo */}
                          {!yaAsignado && grupoMatch && grupoMatch.disponibles>0 && (
                            <button onClick={e=>{ e.stopPropagation(); setAsignarProd(grupoMatch); setShowNotifPanel(false); }}
                              style={{ marginTop:10,width:"100%",padding:"7px",borderRadius:9,background:`linear-gradient(135deg,${C.purple},${C.purpleLight})`,color:C.white,border:"none",fontSize:11,fontWeight:800,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5 }}>
                              <Smartphone size={12}/> Asignar IMEIs ahora ({grupoMatch.disponibles} disponibles)
                            </button>
                          )}
                        </div>
                      );
                  ...
                  {pedidosSinLeer.length>0&&(
                    <div style={{ padding:"10px 18px",borderTop:`1px solid ${C.gray100}` }}>
                      <button onClick={()=>{ pedidosNuevos.forEach((p:any)=>marcarPedidoLeido(p.id)); }}
                        style={{ width:"100%",padding:"8px",borderRadius:9,background:C.purpleBg,color:C.purple,border:`1px solid ${C.purpleBorder}`,fontSize:12,fontWeight:700,cursor:"pointer" }}>
      ...
      <div style={{ maxWidth:1400,margin:"0 auto",padding:"24px" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:24 }}>
          {[
            { l:"Total",       v:stats.total,       bg:C.white,    b:C.gray200,       c:C.gray900    },
            { l:"Disponibles", v:stats.disponibles, bg:C.greenBg,  b:C.greenBorder,   c:C.greenDark  },
            { l:"Vendidos",    v:stats.vendidos,    bg:C.purpleBg, b:C.purpleBorder,  c:C.purple     },
            { l:"En Garantía", v:stats.garantia,    bg:C.orangeBg, b:C.orangeBorder,  c:C.orange     },
            { l:"Defectuosos", v:stats.defectuosos, bg:C.redBg,    b:C.redBorder,     c:C.red        },
          ].map(s=>(
            <div key={s.l} style={{ background:s.bg,borderRadius:14,padding:"14px 16px",border:`1.5px solid ${s.b}` }}>
        ...
        <div style={{ display:"flex",gap:4,background:C.gray100,borderRadius:14,padding:4,marginBottom:20,width:"fit-content" }}>
          {([["producto",Boxes,"Por Producto"],["lote",Layers,"Por Lote"],["buscar",Search,"Buscar IMEI"]] as const).map(([v,Icon,label])=>(
            <button key={v} onClick={()=>setVista(v)}
              style={{ display:"flex",alignItems:"center",gap:7,padding:"9px 18px",borderRadius:10,background:vista===v?C.white:"transparent",border:vista===v?`1px solid ${C.gray200}`:"none",color:vista===v?C.purple:C.gray500,fontSize:13,fontWeight:vista===v?700:500,cursor:"pointer",transition:"all .15s",boxShadow:vista===v?"0 1px 4px rgba(0,0,0,0.08)":"none" }}>
              <Icon size={15} style={{ color:vista===v?C.purple:C.gray500 }} />
              {label}
            </button>
          ))}
        </div>
        ...
        {vista==="producto" && (
          <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
            {loading ? [...Array(4)].map((_,i)=><div key={i} style={{ height:80,borderRadius:14,background:C.white,border:`1px solid ${C.gray200}`,animation:"pulse 1.4s ease infinite" }} />) :
                ...
                <div style={{ display:"flex",alignItems:"center",gap:16,padding:"16px 20px",flexWrap:"wrap" }}>
                  <div style={{ display:"flex",alignItems:"center",gap:12,flex:1,minWidth:200 }}>
                    ...
                    <div>
                      ...
                      <p style={{ margin:"2px 0 0",fontSize:11,color:C.gray500 }}>{g.marca} · SKU: {g.sku} · {g.color} {g.almacenamiento}</p>
                    </div>
                  </div>
                  <div style={{ display:"flex",gap:8,flexWrap:"wrap",alignItems:"center" }}>
                    {[
                      ...
                      ...(g.reservados>0  ? [{ v:g.reservados,  l:"reserv.", bg:"#fffbeb", c:"#b45309", b:"#fde68a"        }] : []),
                      ...(g.defectuosos>0 ? [{ v:g.defectuosos, l:"defect.", bg:C.redBg,  c:C.red,     b:C.redBorder      }] : []),
                    ].map((s:any)=>(
                  ...
                  <div style={{ display:"flex",gap:8,flexShrink:0 }}>
                    ...
                    <button onClick={()=>setDetalleProd(g)}
                      style={{ display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:10,background:C.gray100,color:C.gray700,border:`1px solid ${C.gray200}`,fontSize:12,fontWeight:700,cursor:"pointer" }}
            ...
            {gruposPorLote.map(g=>{
              ...
              return (
                <div key={g.lote} style={{ background:C.white,borderRadius:16,border:`1.5px solid ${C.gray200}`,overflow:"hidden" }}>
                  <button onClick={()=>toggleLote(g.lote)} style={{ width:"100%",display:"flex",alignItems:"center",gap:14,padding:"14px 20px",background:"transparent",border:"none",cursor:"pointer",textAlign:"left" }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <p style={{ margin:"2px 0 0",fontSize:11,color:C.gray500 }}>{g.productoNombre} · Ingreso: {fmtFecha(g.fechaIngreso)}</p>
                    </div>
                    <div style={{ display:"flex",gap:8,alignItems:"center" }}>
                      <div style={{ padding:"4px 10px",borderRadius:20,background:C.greenBg,border:`1px solid ${C.greenBorder}` }}><span style={{ fontSize:12,fontWeight:800,color:C.greenDark }}>{g.disponibles} disp.</span></div>
                      <div style={{ padding:"4px 10px",borderRadius:20,background:C.purpleBg,border:`1px solid ${C.purpleBorder}` }}><span style={{ fontSize:12,fontWeight:800,color:C.purple }}>{g.vendidos} vend.</span></div>
                      <div style={{ padding:"4px 10px",borderRadius:20,background:C.gray100,border:`1px solid ${C.gray200}` }}><span style={{ fontSize:12,fontWeight:800,color:C.gray900 }}>{g.total} total</span></div>
                      {expanded?<ChevronDown size={16} style={{ color:C.gray500 }}/>:<ChevronRight size={16} style={{ color:C.gray500 }}/>}
                    </div>
                  </button>
                  {expanded&&(
                    <div style={{ borderTop:`1px solid ${C.gray100}`,maxHeight:300,overflowY:"auto" }}>
                      <table style={{ width:"100%",borderCollapse:"collapse" }}>
                        <thead><tr style={{ background:C.gray50 }}>{["#","IMEI 1","IMEI 2","Estado","Cliente","Acción"].map(h=><th key={h} style={{ padding:"8px 14px",fontSize:10,fontWeight:700,color:C.gray500,textAlign:"left",textTransform:"uppercase" }}>{h}</th>)}</tr></thead>
                        <tbody>
                          {g.imeis.map((r,i)=>(
                            <tr key={r.id} style={{ borderBottom:`1px solid ${C.gray100}` }}
                              onMouseEnter={e=>(e.currentTarget.style.background="#faf9ff")}
                              onMouseLeave={e=>(e.currentTarget.style.background=C.white)}>
                              <td style={{ padding:"8px 14px",fontSize:11,color:C.gray500 }}>{i+1}</td>
                              <td style={{ padding:"8px 14px",fontFamily:"monospace",fontSize:12,fontWeight:600,color:C.gray900 }}>{r.imei}</td>
                              <td style={{ padding:"8px 14px",fontFamily:"monospace",fontSize:11,color:C.gray500 }}>{r.imei2||"—"}</td>
                              ...
                              <td style={{ padding:"8px 14px" }}>
                                <select value={r.estado} onChange={e=>cambiarEstado(r.id,e.target.value as RegistroIMEI["estado"])}
                                  ...
                                  <option value="vendido">Vendido</option><option value="garantia">Garantía</option><option value="defectuoso">Defectuoso</option>
                                </select>
                              </td>
                            </tr>
                          ))}
        ...
        {vista==="buscar" && (
          <div style={{ display:"flex",flexDirection:"column",gap:16 }}>
            <div style={{ background:C.white,borderRadius:16,padding:"20px 24px",border:`1.5px solid ${C.gray200}` }}>
              <p style={{ margin:"0 0 12px",fontSize:13,fontWeight:700,color:C.gray700 }}>🔍 Busca cualquier IMEI — ideal para garantías y reclamos</p>
              <div style={{ display:"flex",gap:10 }}>
                <div style={{ flex:1,position:"relative" }}>
                  ...
                  <input value={busquedaIMEI} onChange={e=>setBusquedaIMEI(e.target.value)}
                    ...
                    style={{ width:"100%",paddingLeft:40,paddingRight:16,paddingTop:11,paddingBottom:11,borderRadius:12,border:`2px solid ${busquedaIMEI.length>=4?C.purple:C.gray200}`,fontSize:14,fontFamily:"monospace",outline:"none",color:C.gray900 }} />
                </div>
                <select value={filtroEstado} onChange={e=>setFiltroEstado(e.target.value)}
                  ...
                  <option value="reservado">Reservado</option><option value="garantia">Garantía</option><option value="defectuoso">Defectuoso</option>
                </select>
              </div>
            </div>
              ...
              resultadosBusqueda.length===0 ? (
                ...
                <div style={{ background:C.white,borderRadius:16,border:`1.5px solid ${C.gray200}`,overflow:"hidden" }}>
                  <div style={{ padding:"12px 20px",borderBottom:`1px solid ${C.gray100}`,background:C.gray50 }}>
                    <p style={{ margin:0,fontSize:13,fontWeight:700,color:C.gray700 }}>{resultadosBusqueda.length} resultado(s)</p>
                  </div>
                  <table style={{ width:"100%",borderCollapse:"collapse" }}>
                    <thead><tr style={{ background:C.gray50,borderBottom:`1px solid ${C.gray200}` }}>
                      {["IMEI 1","IMEI 2","Producto","Lote","Estado","Cliente","Ingreso","Acción"].map(h=><th key={h} style={{ padding:"10px 14px",fontSize:10,fontWeight:800,color:C.gray500,textAlign:"left",textTransform:"uppercase",whiteSpace:"nowrap" }}>{h}</th>)}
                    </tr></thead>
                    <tbody>
                      {resultadosBusqueda.map((r,i)=>(
                        <tr key={r.id} style={{ borderBottom:i<resultadosBusqueda.length-1?`1px solid ${C.gray100}`:"none" }}
                          onMouseEnter={e=>(e.currentTarget.style.background="#faf9ff")}
                          onMouseLeave={e=>(e.currentTarget.style.background=C.white)}>
                          <td style={{ padding:"12px 14px" }}><span style={{ fontFamily:"monospace",fontSize:13,fontWeight:700,color:C.purple }}>{r.imei}</span></td>
                          <td style={{ padding:"12px 14px",fontFamily:"monospace",fontSize:11,color:C.gray500 }}>{r.imei2||"—"}</td>
                          <td style={{ padding:"12px 14px" }}><p style={{ margin:0,fontSize:13,fontWeight:600,color:C.gray900 }}>{r.productoNombre}</p><p style={{ margin:"1px 0 0",fontSize:10,color:C.gray500 }}>{r.sku}</p></td>
                          <td style={{ padding:"12px 14px" }}><span style={{ fontSize:10,fontWeight:700,padding:"2px 7px",borderRadius:6,background:C.purpleBg,color:C.purple }}>{r.lote}</span></td>
                          ...
                          <td style={{ padding:"12px 14px",fontSize:11,color:C.gray500 }}>{fmtFecha(r.fechaIngreso)}</td>
      ...
      {showRegistrar && <ModalRegistrar onClose={()=>setShowRegistrar(false)} onSaved={()=>showToast("✅ IMEIs registrados")} />}

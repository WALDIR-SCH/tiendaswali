...
interface Producto { id: string; nombre?: string; nombre_producto?: string; precio?: number; stock?: number; stockMinimo?: number; }
interface Pedido   { id: string; numero: string; cliente: string; monto: number; estado: string; fecha?: string; }
...
const ChartTip = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background:"rgba(255,255,255,0.97)", backdropFilter:"blur(12px)", border:"1px solid rgba(152,81,249,0.2)", borderRadius:14, padding:"10px 16px", boxShadow:"0 12px 40px rgba(152,81,249,0.2)" }}>
        ...
        <div style={{ ...card, padding:"26px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
            {productosCriticos.map(p => {
              const s = p.stock ?? 0, m = p.stockMinimo ?? 5;
              ...
              return (
                <div key={p.id} style={{ padding:"11px 13px", borderRadius:12, background:"rgba(255,255,255,0.55)", border:`1px solid ${C.purple}0A` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                    ...
                    <span style={{ fontSize:10, fontWeight:700, padding:"2px 8px", borderRadius:20, flexShrink:0, background: s === 0 ? `${C.orange}15` : `${C.yellow}25`, color: s === 0 ? C.orange : "#a09600", border:`1px solid ${s === 0 ? C.orange : C.yellow}35` }}>
                      {s === 0 ? "Agotado" : `${s}/${m}`}
                    </span>
                  </div>
            ...
            {tasks.map(t => {
              ...
              return (
                <div key={t.id}
                  ...
                  className="task-row"
                  style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 11px", borderRadius:11, cursor:"pointer", background: t.done ? `${C.green}08` : "transparent", transition:"background .15s" }}>
                  <div style={{
                    ...
                    {t.done && <svg style={{ width:10, height:10, color:"#000" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"/></svg>}
                  </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:13 }}>
          {clientes.map((cl, i) => (
            <div key={i} className="client-card" style={{
              ...
              <div style={{ width:46, height:46, borderRadius:14, marginBottom:14, background: cl.c === C.yellow ? C.yellow : `${cl.c}18`, border:`1.5px solid ${cl.c}38`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:19, fontWeight:900, color: cl.c === C.yellow ? "#000" : cl.c }}>
                {cl.n[0]}
              </div>
